...
# --- Hàm normalize để check tên bác sĩ ---
def normalize_str(s: str) -> str:
    """Chuyển về lowercase, bỏ dấu, xóa chấm và khoảng trắng"""
    s = (s or "").lower()
...
async def create_ingress_and_push(room_name: str):
    ...
    print(f"{now()} ✅ Ingress created for {room_name}: {full_rtmp}")

    # chạy ffmpeg stream 1 phút (để demo, có thể tăng)
    cmd = [
        "ffmpeg", "-re",
        "-stream_loop", "-1",
        "-i", video_path,
        ...
        "-c:v", "libx264",
        "-preset", "veryfast",         # CPU nhẹ hơn veryfast
        "-b:v", "1300k",             # giảm xuống 1 Mbps
        "-maxrate", "1500k",
        "-bufsize", "2200k",         # bufsize ~ 2x bitrate
        "-c:a", "aac",
        "-b:a", "96k",               # audio nhẹ hơn
        "-ac", "2",
        "-ar", "22050",
        "-f", "flv", full_rtmp
    ]
    ...
    await asyncio.sleep(3600) # chạy 60s tạm thời xog remove ingress
    if proc.poll() is None:
        proc.terminate()
    await lkapi.ingress.delete_ingress(api.DeleteIngressRequest(ingress_id=ingress.ingress_id))
    print(f"{now()} 🧹 Đã xóa ingress sau khi stream xong.")
...
async def trigger_ingress_if_needed(lkapi, room_name: str, participants):
    """
    Trigger ingress khi chỉ còn 1 user thật trong phòng.
    Sau khi trigger, đánh dấu. Nếu room trống/reset, sẽ cho trigger lại.
    """
    # tự khai báo identities
    ...
    # trigger khi chưa trigger và duy nhất 1 user thật
    if not ingress_state[room_name] and real_count == 1:
        print(f"{now()} 🎯 Room {room_name} có đúng 1 user thật, trigger ingress...")
            ...
            for room in resp.rooms:
                ...
                if num_participants > 0 and room_name not in dispatched_rooms:
                    ...
                    if num_participants == 1:
                        ...
                        # --- Trong loop participants ---
                        normalized_pid = normalize_str(pid)
                        normalized_pname = normalize_str(pname)

                        # ✅ Nếu người đầu tiên là bác sĩ (có 'bs' hoặc 'bacsi' trong tên/identity)
                        if "bs" in normalized_pid or "bs" in normalized_pname or "bacsi" in normalized_pid or "bacsi" in normalized_pname:
